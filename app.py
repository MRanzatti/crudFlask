...
class Usuarios(db.Model):
    ...
    def __init__(self, nome, email, cep, endereco,numero):
        self.nome = nome
        self.email = email
        self.cep = cep
...
def cadastro():
    if request.method == "POST":
        ...
        if nome and email and cep and endereco and numero:
            u = Usuarios(nome,email,cep,endereco, numero)
            db.session.add(u)
            db.session.commit()
...
def mapa():
    ...
    for i in dados:
        ...
        for j in data:
            ...
            pontos = pontos + '\nvar marker_'+ str(linha) +' = new google.maps.Marker({position: {lat: '+ latitude +', lng: '+ longitude +'},map: map,title: \'Position '+ str(linha) +'\' });'
